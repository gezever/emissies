...
function(input, output, session) {
  ...
  #       `$`('Stof') %>%
  #       unique() %>%
  #       sort()<http://id.milieuinfo.be/imjv/emissiepunt/6702#id>
  #   }
  ...
  # Precalculate the breaks we'll need for the two histogramsSELECT ?title   WHERE {?gebied <http://purl.org/dc/terms/title> ?title .   FILTER(?gebied =  <http://id.milieuinfo.be/imjv/emissiepunt/6702#id> )}
  ...
  showZipcodePopup <- function(zipcode, lat, lng) {
    ...
    selectedEmissie <- alle_emissies[alle_emissies$zipcode == zipcode,]
    emissiepuntlabeltabel <- showEmissiepuntLabel(selectedEmissie)
    content <- as.character(tagList(
      tags$h6(emissiepuntlabeltabel$title),
      tags$strong("Rapporteringsjaar:", as.integer(selectedEmissie$jaar)),tags$br(),
      tags$strong(HTML(sprintf("%s, %s %s",
        selectedEmissie$stof, selectedEmissie$hoeveelheid, selectedEmissie$eenheid
      ))), tags$br(),
      tags$a(gsub('[<>]', '', selectedEmissie$refgebied)) 
    ))
  ...
  showEmissiepuntLabel <- function(gebied) {
    ...
    endpoint <- "http://rdfstoreomv-on-1.vm.cumuli.be:3030/blazegraph/namespace/lne/sparql"
    query <- paste("SELECT ?title   WHERE { ?gebied <http://purl.org/dc/terms/title> ?title ; <http://id.milieuinfo.be/def#hoogte> ?hoogte .   FILTER(?gebied = ", toString(gebied$refgebied),")}")
    #query <- "SELECT ?title   WHERE { ?gebied <http://purl.org/dc/terms/title> ?title .   FILTER(?gebied =  <http://id.milieuinfo.be/imjv/emissiepunt/6702#id> )}"
